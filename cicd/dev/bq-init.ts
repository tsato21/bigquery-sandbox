...
async function insertJson(path: string) {
  const table = bq.dataset('app_dataset').table('contractor_list')
  const rows  = JSON.parse(readFileSync(path, 'utf8'))   // ←配列を一気に Parse
...
async function main() {
  ...
  await insertJson('./db/dev/contractors.json')
  console.log('\n✅ BigQuery emulator initialized.')
}
